...
impl<'a> DecompileContext<'a> {
    ...
    fn decompile_element(&mut self, element: &Element) {
        ...
        match element.kind {
            ...
            ElementKind::ConnectionDefinition => self.decompile_definition(element, "connection def"),
            ElementKind::InterfaceDefinition => self.decompile_definition(element, "interface def"),
            ElementKind::AllocationDefinition => self.decompile_definition(element, "allocation def"),
            ElementKind::RequirementDefinition => self.decompile_definition(element, "requirement def"),
            ElementKind::ConstraintDefinition => self.decompile_definition(element, "constraint def"),
            ...
            ElementKind::AnalysisCaseDefinition => self.decompile_definition(element, "analysis def"),
    ...
    fn record_metadata(&mut self, element: &Element) {
        let qn = self.qualified_names.get(&element.id)
            .cloned()
    ...
    fn decompile_definition(&mut self, element: &Element, keyword: &str) {
        ...
        if let Some(name) = &element.name {
            if element.owned_elements.is_empty() && element.documentation.is_none() {
                ...
                self.write_line(&format!("{}{} {}{}{};", abstract_kw, keyword, short, name, specializations));
            } else {
                self.write_line(&format!("{}{} {}{}{} {{", abstract_kw, keyword, short, name, specializations));
                self.decompile_body(element);
                self.write_line("}");
    ...
    fn format_specializations(&self, element_id: &ElementId) -> String {
        let specializations: Vec<&str> = self.model.relationships.iter()
            .filter(|r| &r.source == element_id && r.kind == RelationshipKind::Specialization)
            .filter_map(|r| {
                self.model.elements.get(&r.target)
    ...
    fn format_typing(&self, element_id: &ElementId) -> String {
        let types: Vec<&str> = self.model.relationships.iter()
    ...
    fn get_namespace_imports(&self, element_id: &ElementId) -> Vec<String> {
        self.model.relationships.iter()
            .filter(|r| &r.owner == &Some(element_id.clone()) && r.kind == RelationshipKind::NamespaceImport)
            .filter_map(|r| {
                self.model.elements.get(&r.target)
    ...
    fn get_membership_imports(&self, element_id: &ElementId) -> Vec<String> {
        self.model.relationships.iter()
            .filter(|r| &r.owner == &Some(element_id.clone()) && r.kind == RelationshipKind::MembershipImport)
            .filter_map(|r| {
                self.model.elements.get(&r.target)
                    .and_then(|e| e.qualified_name.as_deref().or(e.name.as_deref()))
                    .map(|n| n.to_string())
    ...
    fn format_subsetting(&self, element_id: &ElementId) -> String {
        let subsets: Vec<&str> = self.model.relationships.iter()
    ...
    fn format_redefinition(&self, element_id: &ElementId) -> String {
        let redefines: Vec<&str> = self.model.relationships.iter()
...
mod tests {
    ...
    fn test_decompile_simple_package() {
        ...
        let pkg = Element::new("pkg-1", ElementKind::Package)
            .with_name("MyPackage");
    ...
    fn test_decompile_with_specialization() {
        ...
        // Base definition
        let base = Element::new("base-1", ElementKind::PartDefinition)
            .with_name("Base");
        model.elements.insert(base.id.clone(), base);
        model.roots.push(ElementId::from("base-1"));
        
        // Derived definition specializing Base
        let derived = Element::new("derived-1", ElementKind::PartDefinition)
            .with_name("Derived");
    ...
    fn test_decompile_with_documentation() {
        ...
        let child = Element::new("dummy", ElementKind::Comment)
            .with_owner(ElementId::from("def-1"));
        ...
        assert!(result.text.contains("doc /* This is a documented element. */"));
    }
    ...
    fn test_decompile_library_package() {
        ...
        let pkg = Element::new("pkg-1", ElementKind::LibraryPackage)
            .with_name("MyLibrary");
        model.elements.insert(pkg.id.clone(), pkg);
        model.roots.push(ElementId::from("pkg-1"));
    ...
    fn test_metadata_preserves_source_info() {
        let model = Model::new();
        let source = SourceInfo::from_path("/path/to/model.xmi")
            .with_format("xmi");
        
        let result = decompile_with_source(&model, source);
        
        assert_eq!(result.metadata.source.path.as_deref(), Some("/path/to/model.xmi"));
    ...
    fn test_decompile_namespace_import() {
        ...
        let target_pkg = Element::new("target-1", ElementKind::Package)
            .with_name("TargetPackage");
        model.elements.insert(target_pkg.id.clone(), target_pkg);
        model.roots.push(ElementId::from("target-1"));
        
        // Package with import
        let mut pkg = Element::new("pkg-1", ElementKind::Package)
            .with_name("MyPackage");
    ...
    fn test_decompile_with_subsetting() {
        ...
        // Base feature
        let base = Element::new("base-1", ElementKind::PartUsage)
            .with_name("basePart");
        model.elements.insert(base.id.clone(), base);
        model.roots.push(ElementId::from("base-1"));
        
        // Derived feature with subsetting
        let derived = Element::new("derived-1", ElementKind::PartUsage)
            .with_name("derivedPart");
        model.elements.insert(derived.id.clone(), derived);
        model.roots.push(ElementId::from("derived-1"));
    ...
    fn test_decompile_with_redefinition() {
        ...
        // Original feature
        let original = Element::new("orig-1", ElementKind::PartUsage)
            .with_name("originalPart");
        model.elements.insert(original.id.clone(), original);
        model.roots.push(ElementId::from("orig-1"));
        
        // Redefining feature
        let redefining = Element::new("redef-1", ElementKind::PartUsage)
            .with_name("redefPart");
        ...
        assert!(result.text.contains("part redefPart redefines originalPart;"));
    }
    ...
    fn test_decompile_usage_with_typing_and_subsetting() {
        ...
        // Type
        let type_def = Element::new("type-1", ElementKind::PartDefinition)
            .with_name("Engine");
        ...
        let feature = Element::new("feat-1", ElementKind::PartUsage)
            .with_name("myEngine");
        ...
        assert!(result.text.contains("part myEngine : Engine subsets basePart;"));
    }
    
    #[test]
    fn test_decompile_xmi_roundtrip() {
        use crate::interchange::{Xmi, ModelFormat};
  ...
  <sysml:Package xmi:id="pkg1" name="VehicleModel">
    ...
    <ownedMember>
      <sysml:PartDefinition xmi:id="pd2" name="Car"/>
        ...
        assert!(result.text.contains("package VehicleModel"), "Missing package: {}", result.text);
        assert!(result.text.contains("part def Vehicle"), "Missing Vehicle: {}", result.text);
        assert!(result.text.contains("part def Car"), "Missing Car: {}", result.text);
        ...
        assert!(result.metadata.get_element("VehicleModel::Vehicle").is_some());
    ...
    fn test_metadata_keyed_by_qualified_name() {
        ...
        model.add_element(Element::new("usage-1", ElementKind::PartUsage)
            .with_name("innerPart")
            .with_owner("def-1"));
        ...
        assert!(result.metadata.get_element("TestPackage").is_some(), 
            "Should have metadata for TestPackage");
        assert!(result.metadata.get_element("TestPackage::OuterDef").is_some(),
            "Should have metadata for TestPackage::OuterDef, got keys: {:?}", 
            result.metadata.elements.keys().collect::<Vec<_>>());
        assert!(result.metadata.get_element("TestPackage::OuterDef::innerPart").is_some(),
            "Should have metadata for TestPackage::OuterDef::innerPart");
        
        // Verify element IDs are preserved
        assert_eq!(
            result.metadata.get_element("TestPackage").unwrap().original_id.as_deref(),
            Some("pkg-1")
        );
        assert_eq!(
            result.metadata.get_element("TestPackage::OuterDef").unwrap().original_id.as_deref(),
            Some("def-1")
        );
        assert_eq!(
            result.metadata.get_element("TestPackage::OuterDef::innerPart").unwrap().original_id.as_deref(),
            Some("usage-1")
        );
    }
}
