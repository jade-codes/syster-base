...
pub fn restore_element_ids(mut model: Model, metadata: &ImportMetadata) -> Model {
    ...
    let qn_to_original_id: HashMap<&str, &str> = metadata.elements.iter()
        .filter_map(|(qn, meta)| {
            meta.original_id.as_deref().map(|id| (qn.as_str(), id))
        })
        .collect();
    
    // Build a mapping from current ID -> original ID based on qualified names
    let id_mapping: HashMap<ElementId, ElementId> = model.elements.iter()
        .filter_map(|(current_id, _element)| {
            // Compute qualified name for this element
            let qn = compute_qualified_name(&model, current_id);
            
            // Look up original ID
            qn_to_original_id.get(qn.as_str()).map(|orig_id| {
                (current_id.clone(), ElementId::new(*orig_id))
            })
        })
        .collect();
...
pub fn restore_ids_from_symbols(mut model: Model, symbol_index: &SymbolIndex) -> Model {
    ...
    let qn_to_id: HashMap<&str, &str> = symbol_index.all_symbols()
    ...
    let id_mapping: HashMap<ElementId, ElementId> = model.elements.iter()
        .filter_map(|(current_id, _element)| {
            ...
            qn_to_id.get(qn.as_str()).map(|sym_id| {
                (current_id.clone(), ElementId::new(*sym_id))
            })
        })
        .collect();
...
fn apply_id_mapping(model: &mut Model, mapping: &HashMap<ElementId, ElementId>) {
    ...
    for (old_id, mut element) in old_elements {
        ...
        element.owned_elements = element.owned_elements.iter()
            .map(|old_child| {
                mapping.get(old_child).cloned().unwrap_or_else(|| old_child.clone())
            })
            .collect();
    ...
    // Remap roots
    model.roots = model.roots.iter()
        .map(|old_id| mapping.get(old_id).cloned().unwrap_or_else(|| old_id.clone()))
        .collect();
...
mod tests {
    ...
    fn test_restore_element_ids_single_element() {
        ...
        model.add_element(
            Element::new("new-id-1", ElementKind::Package)
                .with_name("MyPackage")
        );
        ...
        metadata.add_element(
            "MyPackage",
            ElementMeta::with_id("original-uuid-123"),
        );
        
        let result = restore_element_ids(model, &metadata);
        
        // Element should now have original ID
        assert!(result.elements.contains_key(&ElementId::new("original-uuid-123")));
    ...
    fn test_restore_element_ids_with_ownership() {
        ...
        // Parent package
        let mut pkg = Element::new("pkg-new", ElementKind::Package)
            .with_name("MyPackage");
        ...
        // Check IDs restored
        assert!(result.elements.contains_key(&ElementId::new("pkg-original")));
        assert!(result.elements.contains_key(&ElementId::new("def-original")));
        ...
        assert_eq!(def.owner.as_ref().map(|id| id.as_str()), Some("pkg-original"));
    }
    ...
    fn test_restore_preserves_unmapped_elements() {
        let mut model = Model::new();
        model.add_element(
            Element::new("existing", ElementKind::Package)
                .with_name("Existing")
        );
        model.add_element(
            Element::new("new-only", ElementKind::Package)
                .with_name("NewOnly")
        );
        ...
        assert!(result.elements.contains_key(&ElementId::new("original-existing")));
    ...
    fn test_restore_updates_roots() {
        let mut model = Model::new();
        model.add_element(
            Element::new("root-new", ElementKind::Package)
                .with_name("Root")
        );
...
"#;
        ...
        let mut model = Model::new();
        let mut pkg = Element::new("temp-pkg-id", ElementKind::Package)
            .with_name("TestPkg");
        ...
        assert!(result.elements.contains_key(&ElementId::new("uuid-pkg")),
            "Package should have restored ID");
        assert!(result.elements.contains_key(&ElementId::new("uuid-vehicle")),
        ...
        assert_eq!(vehicle.owner.as_ref().map(|id| id.as_str()), Some("uuid-pkg"));
    }
}
