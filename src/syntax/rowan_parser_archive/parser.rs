...
/// Convert SyntaxKind to a user-friendly name for error messages
...
impl<'a> Parser<'a> {
    fn new(tokens: &'a [Token<'a>], mode: LanguageMode) -> Self {
        Self {
            tokens,
            pos: 0,
    ...
    fn can_start_expression(&self) -> bool {
        matches!(
            ...
            SyntaxKind::NULL_KW | SyntaxKind::L_PAREN | SyntaxKind::L_BRACE | SyntaxKind::AT |
            SyntaxKind::PLUS | SyntaxKind::MINUS | SyntaxKind::TILDE |
            SyntaxKind::NOT_KW | SyntaxKind::ALL_KW | SyntaxKind::NEW_KW |
            SyntaxKind::IF_KW // Conditional expression: if x ? y else z
        )
    }
    ...
    /// Report an "expected X but got Y" error with nice formatting
    fn expected_error(&mut self, expected: &str) {
        let got = if self.at(SyntaxKind::ERROR) {
            "end of file".to_string()
        } else if let Some(tok) = self.current() {
            format!("'{}'", tok.text)
        } else {
            kind_to_name(self.current_kind()).to_string()
        };
        self.error(format!("expected {}, found {}", expected, got));
    }
    
    /// Report an "unexpected X" error with nice formatting
    fn unexpected_error(&mut self, context: &str) {
        let got = if self.at(SyntaxKind::ERROR) {
            "end of file".to_string()
        } else if let Some(tok) = self.current() {
            format!("'{}'", tok.text)
        ...
        self.error(format!("unexpected {} in {}", got, context));
    }
...
impl<'a> grammar::KerMLParser for Parser<'a> {
    ...
    fn parse_body(&mut self) {
        match self.mode {
            LanguageMode::KerML => grammar::kerml::parse_body(self),
            LanguageMode::SysML => {
                // SysML uses its own body parser which handles both KerML and SysML constructs
                ...
                if self.eat(SyntaxKind::SEMICOLON) {
                    ...
                    while self.pos < self.tokens.len() && !self.at(SyntaxKind::R_BRACE) {
                        let start_pos = self.pos;
                        grammar::sysml::parse_package_body_element(self);
                        self.skip_trivia();
                        
                        if self.pos == start_pos && self.pos < self.tokens.len() && !self.at(SyntaxKind::R_BRACE) {
