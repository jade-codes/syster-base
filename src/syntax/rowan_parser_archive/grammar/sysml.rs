...
use crate::syntax::rowan_parser::parser::kind_to_name;
...
pub fn parse_perform_action<P: SysMLParser>(p: &mut P) {
    ...
    if p.at(SyntaxKind::ACTION_KW) {
        ...
        // Parse optional usage_declaration (identification, specialization_part, etc.)
...
/// Per pest: send_node_declaration = { action_node_usage_declaration? ~ send_token ~ (action_body | (node_parameter_member ~ sender_receiver_part? ~ action_body)) }
...
pub fn parse_send_action<P: SysMLParser>(p: &mut P) {
    ...
    // Otherwise, pattern: send <expr> [via <expr>] [to <expr>] ;
...
/// identifier ::> reference | qualified_name
fn parse_connector_end_reference<P: SysMLParser>(p: &mut P) {
    ...
    if p.at_name_token() {
        ...
        // Check for ::> (references operator)
        if p.at(SyntaxKind::COLON_COLON_GT) {
            bump_keyword(p);
...
/// Parse multiplicity: [expression] or [lower..upper]
fn parse_multiplicity<P: SysMLParser>(p: &mut P) {
    p.start_node(SyntaxKind::MULTIPLICITY);
    p.expect(SyntaxKind::L_BRACKET);
    p.skip_trivia();
    
    // Parse multiplicity bounds (simplified - just consume until ])
    while !p.at(SyntaxKind::R_BRACKET) && !p.at(SyntaxKind::ERROR) {
        p.bump();
        p.skip_trivia();
    }
...
pub fn parse_binding_or_succession<P: SysMLParser>(p: &mut P) {
    ...
    // Pattern: binding [mult]? bind [mult]? x = [mult]? y;
    ...
    } else if p.at_name_token() && !p.at(SyntaxKind::FIRST_KW) {
        // Only parse identification if it's not 'first' keyword
        p.parse_identification();
        p.skip_trivia();
        parsed_name = true;
    } else {
...
pub fn parse_sysml_calc_body<P: SysMLParser>(p: &mut P) {
    ...
    if p.at(SyntaxKind::SEMICOLON) {
        ...
        while !p.at(SyntaxKind::R_BRACE) && !p.at(SyntaxKind::ERROR) {
            ...
            // Parameters (in, out, return) - treat as SysML usages to handle ref prefix
