...
pub const DEFINITION_PREFIXES: &[SyntaxKind] = &[
    SyntaxKind::ABSTRACT_KW,
...
pub fn parse_kerml_file<P: KerMLParser>(p: &mut P) {
    ...
    while !p.at(SyntaxKind::ERROR) { // ERROR indicates EOF in our lexer
        p.skip_trivia();
...
pub fn parse_namespace_element<P: KerMLParser>(p: &mut P) {
    ...
    match p.current_kind() {
        ...
        SyntaxKind::AT | SyntaxKind::AT_AT | SyntaxKind::METADATA_KW => parse_annotation(p),
        
        SyntaxKind::CLASS_KW | SyntaxKind::STRUCT_KW | SyntaxKind::DATATYPE_KW |
        ...
        SyntaxKind::END_KW | SyntaxKind::CONST_KW => p.parse_end_feature_or_parameter(),
...
pub fn parse_standalone_relationship<P: KerMLParser>(p: &mut P) {
    ...
    if p.at(SyntaxKind::SPECIALIZATION_KW) {
        ...
        if p.at_name_token() && !p.at_any(&[
            SyntaxKind::SUBCLASSIFIER_KW, SyntaxKind::SUBTYPE_KW, SyntaxKind::SUBSET_KW,
            SyntaxKind::REDEFINITION_KW, SyntaxKind::TYPING_KW,
        ]) {
            parse_identification_and_skip(p);
        }
    }
    ...
    } else if p.at(SyntaxKind::CONJUGATION_KW) {
...
/// Per pest: disjoining = { disjoint_token ~ qualified_reference_chain ~ from_token ~ qualified_reference_chain ~ relationship_body }
...
fn parse_comment_doc_annotation<P: KerMLParser>(p: &mut P) {
    ...
    // Optional identification
    if (p.at_name_token() || p.at(SyntaxKind::LT)) && 
       !p.at(SyntaxKind::ABOUT_KW) && !p.at(SyntaxKind::LOCALE_KW) {
        p.parse_identification();
        p.skip_trivia_except_block_comments();
...
/// Import = 'import' 'all'? ImportedMembership ...
...
pub fn parse_kerml_definition<P: KerMLParser>(p: &mut P) {
    ...
    if p.at_any(&[
        SyntaxKind::CLASS_KW, SyntaxKind::STRUCT_KW, SyntaxKind::DATATYPE_KW,
        SyntaxKind::BEHAVIOR_KW, SyntaxKind::FUNCTION_KW, SyntaxKind::CLASSIFIER_KW,
        SyntaxKind::INTERACTION_KW, SyntaxKind::PREDICATE_KW, SyntaxKind::METACLASS_KW,
        SyntaxKind::TYPE_KW,
    ]) {
        p.bump();
...
fn parse_calc_body_element<P: KerMLParser>(p: &mut P) -> bool {
    if p.at_any(&[SyntaxKind::IN_KW, SyntaxKind::OUT_KW, SyntaxKind::INOUT_KW, SyntaxKind::RETURN_KW]) {
        ...
        SyntaxKind::CALC_KW, SyntaxKind::CONSTRAINT_KW,
        ...
        true
              ...
              p.at(SyntaxKind::INTEGER) || p.at(SyntaxKind::STRING) {
        super::kerml_expressions::parse_expression(p);
        p.skip_trivia();
        if p.at(SyntaxKind::SEMICOLON) {
            p.bump();
...
/// Parse feature prefix modifiers (var, composite, etc.)
fn parse_feature_prefix_modifiers<P: KerMLParser>(p: &mut P) {
    while p.at_any(&[
        SyntaxKind::VAR_KW, SyntaxKind::COMPOSITE_KW, SyntaxKind::PORTION_KW,
        SyntaxKind::MEMBER_KW, SyntaxKind::ABSTRACT_KW, SyntaxKind::DERIVED_KW,
    ]) {
        p.bump();
        p.skip_trivia();
    }
...
fn parse_optional_feature_keyword<P: KerMLParser>(p: &mut P) -> bool {
    if p.at_any(&[SyntaxKind::FEATURE_KW, SyntaxKind::STEP_KW, SyntaxKind::EXPR_KW, SyntaxKind::INV_KW]) {
        p.bump();
        true
...
/// Parse optional default value (= or :=)
...
pub fn parse_kerml_parameter<P: KerMLParser>(p: &mut P) {
    ...
    if p.at_any(&[
        ...
        SyntaxKind::PORT_KW, SyntaxKind::FEATURE_KW, SyntaxKind::STEP_KW,
        SyntaxKind::EXPR_KW,
    ]) {
        bump_and_skip(p);
    }
...
fn parse_prefix_modifiers<P: KerMLParser>(p: &mut P) {
    while p.at_any(&[
        ...
        SyntaxKind::ABSTRACT_KW, SyntaxKind::VARIATION_KW, SyntaxKind::VAR_KW,
        SyntaxKind::COMPOSITE_KW, SyntaxKind::PORTION_KW, SyntaxKind::INDIVIDUAL_KW
    ]) {
        bump_and_skip(p);
    }
}
...
fn parse_optional_sysml_usage_keyword<P: KerMLParser>(p: &mut P) {
    if p.at_any(&[
        SyntaxKind::ITEM_KW, SyntaxKind::PART_KW, SyntaxKind::ACTION_KW,
        SyntaxKind::STATE_KW, SyntaxKind::ATTRIBUTE_KW, SyntaxKind::PORT_KW
    ]) {
        bump_and_skip(p);
    }
}
...
fn parse_end_feature_with_keyword<P: KerMLParser>(p: &mut P) {
    ...
    if p.at(SyntaxKind::REDEFINES_KW) || p.at(SyntaxKind::COLON_GT_GT) ||
       p.at(SyntaxKind::SUBSETS_KW) || p.at(SyntaxKind::COLON_GT) ||
       p.at(SyntaxKind::REFERENCES_KW) || p.at(SyntaxKind::COLON_COLON_GT) {
...
fn parse_connector_name_or_specialization<P: KerMLParser>(p: &mut P, looks_like_direct_endpoint: bool) {
    if p.at(SyntaxKind::COLON_GT) || p.at(SyntaxKind::COLON_GT_GT)
        || p.at(SyntaxKind::SUBSETS_KW) || p.at(SyntaxKind::SPECIALIZES_KW)
...
pub fn parse_connector_usage<P: KerMLParser>(p: &mut P) {
    ...
    while p.at_any(&[
        SyntaxKind::VAR_KW, SyntaxKind::COMPOSITE_KW, SyntaxKind::PORTION_KW,
        SyntaxKind::MEMBER_KW, SyntaxKind::ABSTRACT_KW, SyntaxKind::DERIVED_KW,
        SyntaxKind::CONST_KW, SyntaxKind::END_KW,
    ]) {
        bump_and_skip(p);
    }
    
    // Dispatch to binding/succession if applicable
    if p.at_any(&[SyntaxKind::BINDING_KW, SyntaxKind::SUCCESSION_KW, SyntaxKind::FIRST_KW]) {
    ...
    let looks_like_direct = looks_like_qualified_name_before(p, &[SyntaxKind::TO_KW, SyntaxKind::FROM_KW]);
...
fn parse_binding_succession_prefix<P: KerMLParser>(p: &mut P, looks_like_direct_endpoint: bool) -> bool {
...
fn parse_binding_or_succession_impl<P: KerMLParser>(p: &mut P) {
    ...
    let parsed_name = if should_parse_first_pattern(p, is_succession) {
        ...
        let looks_like_direct = looks_like_qualified_name_before(p, &[SyntaxKind::EQ, SyntaxKind::THEN_KW]);
...
pub fn parse_feature_relationships<P: KerMLParser>(p: &mut P) {
    while p.at_any(&[
        SyntaxKind::FEATURED_KW, SyntaxKind::INVERSE_KW,
...
mod tests {
    ...
    #[test]
    fn test_kerml_definition_keywords() {
        assert!(is_kerml_definition_keyword(SyntaxKind::CLASS_KW));
        assert!(is_kerml_definition_keyword(SyntaxKind::STRUCT_KW));
        assert!(is_kerml_definition_keyword(SyntaxKind::FUNCTION_KW));
        assert!(!is_kerml_definition_keyword(SyntaxKind::PART_KW));
    }
    
    #[test]
    fn test_kerml_usage_keywords() {
        assert!(is_kerml_usage_keyword(SyntaxKind::FEATURE_KW));
        assert!(is_kerml_usage_keyword(SyntaxKind::STEP_KW));
        assert!(!is_kerml_usage_keyword(SyntaxKind::PART_KW));
    }
    
    #[test]
    fn test_standalone_relationship_keywords() {
        assert!(is_standalone_relationship_keyword(SyntaxKind::SPECIALIZATION_KW));
        assert!(is_standalone_relationship_keyword(SyntaxKind::SUBCLASSIFIER_KW));
        assert!(!is_standalone_relationship_keyword(SyntaxKind::PART_KW));
    }
}
