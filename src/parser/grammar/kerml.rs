...
pub fn parse_namespace_element<P: KerMLParser>(p: &mut P) {
    ...
    match p.current_kind() {
        ...
        SyntaxKind::TYPING_KW | SyntaxKind::CONJUGATION_KW | SyntaxKind::CONJUGATE_KW |
...
fn parse_feature_prefix_modifiers<P: KerMLParser>(p: &mut P) {
    while p.at_any(&[
        ...
        SyntaxKind::CONST_KW, SyntaxKind::END_KW, SyntaxKind::VARIATION_KW,
    ]) {
        p.bump();
        p.skip_trivia();
    }
...
fn parse_optional_feature_keyword<P: KerMLParser>(p: &mut P) -> bool {
    if p.at(SyntaxKind::INV_KW) {
        ...
        true
    } else if p.at_any(&[SyntaxKind::FEATURE_KW, SyntaxKind::STEP_KW, SyntaxKind::EXPR_KW]) {
        p.bump();
        true
