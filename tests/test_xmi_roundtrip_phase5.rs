...
mod roundtrip_tests {
    ...
    use syster::interchange::{
        Model, ModelFormat, Xmi, 
        model_from_symbols, symbols_from_model,
        decompile, apply_metadata_to_host,
    };
    use std::sync::Arc;
...
"#;
        ...
        assert_eq!(symbols_v1.len(), symbols_v2.len(), "Symbol count should match");
        assert_eq!(symbols_v2.len(), symbols_v3.len(), "Symbol count should match");
        ...
        for i in 0..symbols_v1.len() {
            ...
            assert_eq!(s2.qualified_name, s3.qualified_name, 
                "Qualified names should match across cycles");
            
            // Element IDs should be stable
            assert_eq!(s1.element_id, s2.element_id, 
                "Element ID for {} should be stable after round-trip", s1.qualified_name);
            assert_eq!(s2.element_id, s3.element_id, 
                "Element ID for {} should be stable after multiple round-trips", s1.qualified_name);
        }
...
"#;
        ...
        for orig_symbol in &original_symbols {
            let found = all_symbols.iter()
                .find(|s| s.qualified_name == orig_symbol.qualified_name);
            
            if let Some(found) = found {
                assert_eq!(found.element_id, orig_symbol.element_id,
        ...
        let new_component = all_symbols.iter()
        ...
        for orig_symbol in &original_symbols {
            assert_ne!(new_id, &orig_symbol.element_id,
...
"#;
        ...
        let element_ids1: Vec<Arc<str>> = symbols1.iter()
        ...
        for (s1, s2) in s1_sorted.iter().zip(s2_sorted.iter()) {
            assert_eq!(s1.qualified_name, s2.qualified_name, "Qualified names should match");
            assert_eq!(s1.element_id, s2.element_id, 
        ...
        for id in &element_ids1 {
            assert!(xmi1_str.contains(id.as_ref()), "First XMI should contain {}", id);
            assert!(xmi2_str.contains(id.as_ref()), "Second XMI should contain {}", id);
        }
    }
...
"#;
        ...
        let combined_xmi = Xmi.write(&combined_model).expect("Should write combined XMI");
        ...
        for imported in &imported_symbols {
            ...
            assert!(found.is_some(), "Imported symbol {} should exist", imported.qualified_name);
            assert_eq!(found.unwrap().element_id, imported.element_id,
                "Imported element ID for {} should be preserved",
                imported.qualified_name);
        }
        
        // New elements should have different IDs
        let new_part = all_symbols.iter()
            .find(|s| s.name.as_ref() == "NewPart");
        
        if let Some(new_part) = new_part {
            for imported in &imported_symbols {
                assert_ne!(new_part.element_id, imported.element_id,
        ...
        for imported in &imported_symbols {
            assert!(combined_xmi_str.contains(imported.element_id.as_ref()),
...
"#;
        ...
        decompile_result.metadata.write_to_file(&metadata_path).unwrap();
        ...
        for symbol in &symbols1 {
            ...
            assert!(xmi2_str.contains(id), "Re-exported XMI should contain {}", id);
        }
    }
}
