...
fn test_assert_constraint() {
    ...
    let has_speedlimit_ref = refs.iter().any(|(_, targets)| targets.contains(&"SpeedLimit".to_string()));
    
    assert!(has_speedlimit_ref, "Should have reference to 'SpeedLimit', got: {:?}", refs);
}
...
fn test_assume_constraint() {
    ...
    let has_margin_ref = refs.iter().any(|(_, targets)| targets.contains(&"SafetyMargin".to_string()));
    
    assert!(has_margin_ref, "Should have reference to 'SafetyMargin', got: {:?}", refs);
}
...
fn test_require_constraint() {
    ...
    let has_range_ref = refs.iter().any(|(_, targets)| targets.contains(&"ValidRange".to_string()));
    
    assert!(has_range_ref, "Should have reference to 'ValidRange', got: {:?}", refs);
}
