...
fn test_xmi_to_symbols_preserves_element_ids() {
    use syster::interchange::{Model, Element, ElementKind, Xmi, ModelFormat, symbols_from_model, model_from_symbols, restore_ids_from_symbols};
    use syster::hir::SymbolIndex;

    // 1. Create a test XMI model
    let mut model = Model::new();
    let pkg = Element::new("test-pkg-123", ElementKind::Package)
        .with_name("TestPackage");
    model.add_element(pkg);
    
    let part_def = Element::new("test-part-456", ElementKind::PartDefinition)
        .with_name("Vehicle");
    ...
    let pkg_symbol = symbols.iter().find(|s| s.name.as_ref() == "TestPackage")
        .expect("Should find TestPackage symbol");
    assert_eq!(pkg_symbol.element_id.as_ref(), "test-pkg-123", 
        "Package element_id should be preserved");
    
    let part_symbol = symbols.iter().find(|s| s.name.as_ref() == "Vehicle")
        .expect("Should find Vehicle symbol");
    assert_eq!(part_symbol.element_id.as_ref(), "test-part-456",
    ...
    assert!(model3.elements.iter().any(|(_, e)| 
        e.id.as_str() == "test-pkg-123" && e.name.as_deref() == Some("TestPackage")
    ), "Exported model should have original package ID");
    
    assert!(model3.elements.iter().any(|(_, e)| 
        e.id.as_str() == "test-part-456" && e.name.as_deref() == Some("Vehicle")
...
fn test_import_into_host_preserves_ids() {
    ...
    let mut model = Model::new();
    model.add_element(
        Element::new("xmi-id-001", ElementKind::Package)
            .with_name("RootPackage")
    );
    model.add_element(
        Element::new("xmi-id-002", ElementKind::PartDefinition)
            .with_name("Component")
    );
    ...
    let root_pkg = all_symbols.iter().find(|s| s.name.as_ref() == "RootPackage")
        .expect("Should find RootPackage");
    assert_eq!(root_pkg.element_id.as_ref(), "xmi-id-001",
        "RootPackage element_id should match XMI");
    
    let component = all_symbols.iter().find(|s| s.name.as_ref() == "Component")
        .expect("Should find Component");
    assert_eq!(component.element_id.as_ref(), "xmi-id-002",
