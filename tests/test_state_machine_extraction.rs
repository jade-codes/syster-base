...
fn test_then_simple_transition() {
    ...
    let has_s1_ref = refs.iter().any(|(_, targets)| targets.contains(&"s1".to_string()));
    let has_s2_ref = refs.iter().any(|(_, targets)| targets.contains(&"s2".to_string()));
...
fn test_then_in_state_body() {
    ...
    let has_running_ref = refs.iter().any(|(_, targets)| targets.contains(&"running".to_string()));
    
    assert!(has_running_ref, "Should have reference to 'running', got: {:?}", refs);
}
...
fn test_first_state() {
    ...
    let has_idle_ref = refs.iter().any(|(_, targets)| targets.contains(&"idle".to_string()));
    
    assert!(has_idle_ref, "Should have reference to 'idle', got: {:?}", refs);
}
...
fn test_accept_signal() {
    ...
    let has_signal_ref = refs.iter().any(|(_, targets)| targets.contains(&"Signal".to_string()));
    let has_running_ref = refs.iter().any(|(_, targets)| targets.contains(&"running".to_string()));
    
    assert!(has_signal_ref, "Should have reference to 'Signal', got: {:?}", refs);
    assert!(has_running_ref, "Should have reference to 'running', got: {:?}", refs);
}
...
fn test_accept_with_when() {
    ...
    let has_alarm_ref = refs.iter().any(|(_, targets)| targets.contains(&"alarm".to_string()));
    
    assert!(has_alarm_ref, "Should have reference to 'alarm', got: {:?}", refs);
}
