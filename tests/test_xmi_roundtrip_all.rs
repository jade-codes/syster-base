...
fn test_xmi_roundtrip_all_library_files() {
    ...
    assert!(
        !xmi_files.is_empty(),
        "Should find XMI files in the release"
    );
    ...
    for file_path in &xmi_files {
        ...
        println!(
            ...
            file_name, original_count, original_rels
        );
        passed += 1;
    }
...
fn test_xmi_roundtrip_with_direction_attributes() {
    ...
    for rel_path in test_files {
        ...
        assert!(!model.elements.is_empty(), "Should have elements");
    }
}
...
fn test_xmi_roundtrip_preserves_element_ids() {
    ...
    let original_ids: std::collections::HashSet<_> = model
        .elements
        .keys()
    ...
    let reimported_ids: std::collections::HashSet<_> = reimported_model
        .elements
        .keys()
        .map(|id| id.as_str().to_string())
        .collect();
