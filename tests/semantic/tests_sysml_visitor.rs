...
fn test_same_name_in_different_namespaces_creates_two_symbols() {
    ...
    let shell_symbols: Vec<_> = symbol_table
        .iter_symbols()
        .filter(|sym| sym.name() == "Shell")
        .collect();
...
fn test_comma_separated_redefinitions_do_not_create_duplicate_symbols() {
    ...
    let disc_count = symbol_table
        .iter_symbols()
        .filter(|sym| sym.name() == "Disc")
        .count();
    let circle_count = symbol_table
        .iter_symbols()
...
fn test_simple_redefinition_creates_child_symbol() {
    ...
    let radius_symbols: Vec<_> = symbol_table
        .iter_symbols()
        .filter(|sym| sym.name() == "radius")
        .collect();
...
fn test_visitor_records_specialization_relationship() {
    ...
    // get_sources("Vehicle") returns sources that reference Vehicle
...
fn test_visitor_records_typing_relationship() {
    ...
    // myCar references Vehicle via typing
    // get_sources("Vehicle") returns sources that reference Vehicle
    let sources = graph.get_sources("Vehicle");
    assert!(
        sources.contains(&"myCar"),
        "myCar should be in sources for Vehicle"
    );
}
...
fn test_multiple_specializations() {
    ...
    // ElectricCar references Car, Electric, and Vehicle
    // Each target's sources should include ElectricCar
    assert!(graph.get_sources("Car").contains(&"ElectricCar"));
    assert!(graph.get_sources("Electric").contains(&"ElectricCar"));
    assert!(graph.get_sources("Vehicle").contains(&"ElectricCar"));
}
...
fn test_scoped_symbols_with_same_name() {
    ...
    let speed_symbols: Vec<_> = symbol_table
        .iter_symbols()
...
fn test_multiple_usages_of_same_type() {
    ...
    for name in ["car1", "car2", "car3"] {
        ...
        assert!(
            ...
            "{name} should be in sources for Vehicle"
        );
    }
}
...
fn test_redefinition_relationship() {
    ...
    // SportsCar references Car via redefinition
    // get_sources("Car") should contain SportsCar
    let sources = graph.get_sources("Car");
    assert!(
        sources.contains(&"SportsCar"),
        "SportsCar should be in sources for Car"
    );
}
...
fn test_action_with_parameters() {
    ...
    let has_data = symbol_table.iter_symbols().any(|sym| sym.name() == "data");
    let has_result = symbol_table
        .iter_symbols()
        .any(|sym| sym.name() == "result");
...
fn test_enumeration_definition() {
    ...
    let has_red = symbol_table.iter_symbols().any(|sym| sym.name() == "Red");
    let has_green = symbol_table.iter_symbols().any(|sym| sym.name() == "Green");
    let has_blue = symbol_table.iter_symbols().any(|sym| sym.name() == "Blue");
...
fn test_mixed_definitions_and_usages() {
    ...
    assert!(
        symbol_table
            .iter_symbols()
            .any(|sym| sym.name() == "engine")
    );
    assert!(
        symbol_table
            .iter_symbols()
            .any(|sym| sym.name() == "wheel1")
    );
    assert!(
        symbol_table
            .iter_symbols()
            .any(|sym| sym.name() == "wheel2")
    );
}
