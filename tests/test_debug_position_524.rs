...
fn debug_position_line_524() {
    ...
    println!("\n=== Line {} (0-indexed: {}) ===", target_line, target_line - 1);
    ...
    println!("\n=== Context (lines {}-{}) ===", target_line - 3, target_line + 2);
        ...
        for trk in &sym.type_refs {
            for tr in trk.as_refs() {
                if tr.target.as_ref() == "fuel" && tr.start_line >= 517 && tr.start_line <= 524 {
                    ...
                    println!("  line={} col={}-{} kind={:?} resolved={:?}", 
    ...
    println!("\n=== Symbols with type_refs at line {} (0-indexed), col {} ===", search_line, target_col);
    for sym in index.symbols_in_file(FileId::new(0)) {
        for trk in &sym.type_refs {
            for tr in trk.as_refs() {
                // Check if this type_ref covers our position
                if tr.start_line == search_line && tr.start_col <= target_col && tr.end_col >= target_col {
    ...
    println!("\n=== Also checking with 1-indexed line {} ===", target_line);
    for sym in index.symbols_in_file(FileId::new(0)) {
        for trk in &sym.type_refs {
            for tr in trk.as_refs() {
                if tr.start_line == target_line && tr.start_col <= target_col && tr.end_col >= target_col {
    ...
    println!("\n=== find_type_ref_at_position at line {} col {} (0-indexed) ===", target_line - 1, target_col);
    if let Some(ctx) = find_type_ref_at_position(&index, FileId::new(0), target_line - 1, target_col) {
