...
fn test_reparse_preserves_element_ids() {
    ...
    let pkg1 = analysis1.symbol_index().lookup_qualified("TestPackage")
        .expect("Should find TestPackage");
    let initial_pkg_id = pkg1.element_id.clone();
    
    let vehicle1 = analysis1.symbol_index().lookup_qualified("TestPackage::Vehicle")
    ...
    assert!(!initial_vehicle_id.is_empty(), "Vehicle should have element_id");
    ...
    let pkg2 = analysis2.symbol_index().lookup_qualified("TestPackage")
        .expect("Should still find TestPackage");
    let vehicle2 = analysis2.symbol_index().lookup_qualified("TestPackage::Vehicle")
        .expect("Should still find Vehicle");
    
    // 3. Verify element IDs were preserved
    assert_eq!(pkg2.element_id, initial_pkg_id,
        "TestPackage element_id should be preserved across re-parse");
    assert_eq!(vehicle2.element_id, initial_vehicle_id,
        "Vehicle element_id should be preserved across re-parse");
    
    // New symbol should get a new ID
    let mass = analysis2.symbol_index().lookup_qualified("TestPackage::Vehicle::mass")
        .expect("Should find new attribute 'mass'");
    assert!(!mass.element_id.is_empty(), "New attribute should have element_id");
    assert_ne!(mass.element_id, initial_pkg_id, "New symbol should have different ID");
}
...
fn test_imported_symbols_preserve_ids_on_reparse() {
    use syster::ide::AnalysisHost;
    use syster::interchange::{Model, Element, ElementKind, symbols_from_model};
    
    // 1. Import XMI with known IDs
    let mut model = Model::new();
    model.add_element(
        Element::new("xmi-imported-001", ElementKind::Package)
            .with_name("ImportedPackage")
    );
    model.add_element(
        Element::new("xmi-imported-002", ElementKind::PartDefinition)
            .with_name("ImportedPart")
    );
    ...
    let pkg1 = analysis1.symbol_index().lookup_qualified("ImportedPackage")
        .expect("Should find imported package");
    assert_eq!(pkg1.element_id.as_ref(), "xmi-imported-001",
    ...
    if pkg2_opt.is_none() {
        panic!("ImportedPackage not found after rebuild! Available: {:?}", 
            all_syms2.iter().map(|s| s.qualified_name.as_ref()).collect::<Vec<_>>());
    }
    let pkg2 = pkg2_opt.unwrap();
    assert_eq!(pkg2.element_id.as_ref(), "xmi-imported-001",
        "Imported package element_id should survive index rebuild");
    
    let part2 = analysis2.symbol_index().lookup_qualified("ImportedPart")
        .expect("Should still find imported part after rebuild");
    assert_eq!(part2.element_id.as_ref(), "xmi-imported-002",
...
fn test_symbol_removed_and_readded() {
    ...
    let analysis1 = host.analysis();
    let a1 = analysis1.symbol_index().lookup_qualified("Pkg::A")
    ...
    let analysis2 = host.analysis();
    assert!(analysis2.symbol_index().lookup_qualified("Pkg::A").is_none(),
        "A should be removed");
    
    // 3. Re-add the symbol - should get the SAME ID back
    host.set_file_content("test.sysml", "package Pkg { part def A; }");
    let analysis3 = host.analysis();
    let a3 = analysis3.symbol_index().lookup_qualified("Pkg::A")
        .expect("Should find A again");
    
    assert_eq!(a3.element_id, original_id,
