...
"#;
        ...
        let has_by_target = satisfy_sym.type_refs.iter().any(|tr: &TypeRefKind| {
            tr.as_refs()
                .iter()
                .any(|r| r.target.as_ref() == "vehicle_b")
        });
        assert!(
            has_by_target,
            "satisfy symbol should have vehicle_b as type_ref"
        );
        ...
        if let Some(ref_) = vehicle_b_ref {
            ...
            assert!(
                hover.is_some(),
                "hover on 'vehicle_b' by-target should resolve"
            );
            let qn = hover.unwrap().qualified_name;
            assert!(
                qn.as_ref()
                    .map(|s| s.contains("vehicle_b"))
                    .unwrap_or(false),
                "hover should resolve to vehicle_b, got {:?}",
                qn
            );
        } else {
...
"#;
        ...
        let has_via_target = accept_sym.type_refs.iter().any(|tr: &TypeRefKind| {
            tr.as_refs()
                .iter()
                .any(|r| r.target.as_ref() == "ignitionPort")
        });
        assert!(
            has_via_target,
            "accept symbol should have ignitionPort as type_ref"
        );

        // Find the via ref and test hover
        let via_ref = accept_sym
            .type_refs
            .iter()
            .flat_map(|tr: &TypeRefKind| tr.as_refs())
        ...
        if let Some(ref_) = via_ref {
            ...
            assert!(
                qn.as_ref()
                    .map(|s| s.contains("ignitionPort"))
                    .unwrap_or(false),
                "hover should resolve to ignitionPort, got {:?}",
                qn
            );
        }
    }
...
"#;
        ...
        let has_via_target = accept_sym.type_refs.iter().any(|tr: &TypeRefKind| {
            tr.as_refs()
                .iter()
                .any(|r| r.target.as_ref() == "ignitionCmdPort")
        });
        assert!(
            has_via_target,
            "accept symbol should have ignitionCmdPort as type_ref from via clause"
        );
        ...
        if let Some(ref_) = via_ref {
            ...
            assert!(
                hover.is_some(),
                "hover on transition via target should resolve"
            );
            let qn = hover.unwrap().qualified_name;
            assert!(
                qn.as_ref()
                    .map(|s| s.contains("ignitionCmdPort"))
                    .unwrap_or(false),
                "hover should resolve to ignitionCmdPort, got {:?}",
                qn
            );
        }
    }
