...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on redefines target 'providePower'");
...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on qualified ref 'ActionTree::providePower'");
    println!("Hover result: {:?}", hover);
}
...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on feature chain first part 'fuelTank'");
    println!("Hover result: {:?}", hover);
}
...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on feature chain second part 'mass'");
    println!("Hover result: {:?}", hover);
}
...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on subsets target 'parentAction'");
    println!("Hover result: {:?}", hover);
}
...
fn test_hover_on_transition_target() {
    use syster::parser::{parse_sysml, ast::{AstNode, NamespaceMember, SourceFile}};
...
"#;
            ...
            if let Some(body) = pkg.body() {
                for inner in body.members() {
                    ...
                    if let NamespaceMember::Definition(def) = inner {
                        println!("  Definition body: {:?}", def.body().map(|b| b.syntax().kind()));
                        println!("  Definition children count: {}", def.body().map(|b| b.members().count()).unwrap_or(0));
    ...
    assert!(hover.is_some(), "Expected hover on transition target 'running'");
}
...
"#;
    ...
    assert!(hover.is_some(), "Expected hover on expression reference 'y'");
    println!("Hover result: {:?}", hover);
}
// =============================================================================
...
"#;
    ...
    println!("Hover on 'turnVehicleOn' (col 63): {:?}", hover_turn.is_some());
    
    // Also test `trigger1`
    let hover_trigger = has_hover_at(&mut host, 14, 87);
    println!("Hover on 'trigger1' (col 87): {:?}", hover_trigger.is_some());
    
    assert!(hover_driver.is_some(), "Expected hover on 'driver'");
    assert!(hover_turn.is_some(), "Expected hover on 'turnVehicleOn' - this is the chain member");
    assert!(hover_trigger.is_some(), "Expected hover on 'trigger1' - this is the chain member");
}
...
"#;
    ...
    println!("Hover on second 'shaftPort_d' (chain): {:?}", hover3.is_some());
    
    assert!(hover3.is_some(), "Expected hover on 'differential.shaftPort_d' chain member");
}
...
"#;
    ...
    println!("\n--- ALL SYMBOLS ---");
    {
        ...
        for sym in index.symbols_in_file(file_id) {
            println!("  {} ({:?})", sym.qualified_name, sym.kind);
            println!("    span: line {}-{}, col {}-{}", sym.start_line, sym.end_line, sym.start_col, sym.end_col);
            if !sym.type_refs.is_empty() {
                for (i, tr) in sym.type_refs.iter().enumerate() {
                    println!("    type_ref[{}]: {:?}", i, tr);
                }
    ...
    // All should resolve
    assert!(hover1.is_some(), "Expected hover on bind target 'shaftPort_d'");
    assert!(hover2.is_some(), "Expected hover on 'differential'");
    assert!(hover3.is_some(), "Expected hover on chain member 'shaftPort_d'");
}
...
"#;
    ...
    assert!(h1.contains("initial") || h1.contains("::initial"), 
        "Hover should mention 'initial', got: {}", h1);
    
    assert!(hover2.is_some(), "Expected hover on transition target 'off'");
    let h2 = hover2.unwrap();
    assert!(h2.contains("off") || h2.contains("::off"),
        "Hover should mention 'off', got: {}", h2);
}
        ...
        for child in node.children_with_tokens() {
            match child {
                ...
                rowan::NodeOrToken::Token(t) => {
                    println!("{}  {:?} {:?} [{:?}]", indent_str, t.kind(), t.text(), t.text_range());
                }
            }
        }
    }
