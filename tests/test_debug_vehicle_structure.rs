...
fn debug_vehicle_structure() {
    ...
    if let Some(sym) = index
        .lookup_qualified("SimpleVehicleModel::Definitions::PartDefinitions::Vehicle::providePower")
    {
    ...
    let chain_parts: Vec<std::sync::Arc<str>> =
        vec!["providePower".into(), "distributeTorque".into()];
    ...
    for (i, line) in lines.iter().enumerate() {
        if line.contains("perform providePower.distributeTorque") {
            if let Some(pos) = line.find("distributeTorque") {
                ...
                println!(
                    "\nFound 'distributeTorque' at line {} col {} in: {}",
                    target_line,
                    target_col,
                    line.trim()
                );
    ...
    assert!(
        ...
        "Should get hover for distributeTorque"
    );
}
