...
fn main() {
    ...
    println!("Imported: {} elements, {} relationships", 
        model.elements.len(), model.relationships.len());
    ...
    println!("Re-imported: {} elements, {} relationships",
        model2.elements.len(), model2.relationships.len());
    
    // Compare
    println!("\n=== Comparison ===");
    println!("Elements match: {}", model.elements.len() == model2.elements.len());
    println!("Relationships match: {}", model.relationships.len() == model2.relationships.len());
    ...
    for (id, elem1) in &model.elements {
        if let Some(elem2) = model2.elements.get(id) {
            matching_ids += 1;
            if elem1.name == elem2.name { matching_names += 1; }
            if elem1.kind == elem2.kind { matching_kinds += 1; }
        }
    }
    
    println!("\nElement-by-element comparison:");
    println!("  Matching IDs: {}/{}", matching_ids, model.elements.len());
    println!("  Matching names: {}/{}", matching_names, model.elements.len());
    println!("  Matching kinds: {}/{}", matching_kinds, model.elements.len());
