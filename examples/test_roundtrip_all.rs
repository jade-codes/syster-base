    ...
    for entry in WalkDir::new(base_path).into_iter().filter_map(|e| e.ok()) {
        ...
        if let Some(ext) = path.extension() {
            if ext == "sysmlx" || ext == "kermlx" {
                ...
                let names1: std::collections::HashSet<_> = model1.elements.values()
                    .filter_map(|e| e.name.clone())
                    .collect();
                let names2: std::collections::HashSet<_> = model2.elements.values()
                    .filter_map(|e| e.name.clone())
                    .collect();
                if names1 == names2 {
                    name_match += 1;
                ...
                let kinds1: std::collections::HashSet<_> = model1.elements.values()
                    .map(|e| (e.id.clone(), e.kind))
                    .collect();
                let kinds2: std::collections::HashSet<_> = model2.elements.values()
                    .map(|e| (e.id.clone(), e.kind))
                    .collect();
                if kinds1 == kinds2 {
                    kind_match += 1;
